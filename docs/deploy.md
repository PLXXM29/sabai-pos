...
| `DEMO_RESET_EVERY` | | `24h` | อายุสูงสุดของชุดข้อมูลก่อนถูกสร้างใหม่ (`0` = ไม่ทำ) เดโมสาธารณะตั้งไว้ `3h` เพราะข้อมูลจำลองการค้าถึงแค่ตอนที่มันถูกสร้าง ทิ้งไว้นานกว่านั้น "ยอดขายวันนี้" จะค้างอยู่กับอดีต · สร้างใหม่ทุกครั้งที่วันที่ตามเวลาไทยเปลี่ยนด้วย · วัดจากเวลาในฐานข้อมูล ไม่ใช่ uptime ของโปรเซส เพราะแอป scale-to-zero |
